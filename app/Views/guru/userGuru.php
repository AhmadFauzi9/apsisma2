    ...
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
            <ol class="breadcrumb bg-transparent px-0 my-0 py-0">
                ...
                <div class="breadcrumb-item"><a href="<?= base_url() ?>/siswa/userSiswa/">Users Guru</a></div>
            </ol>
        </div>
    </div>
</section>
...
<!-- KONTAK GURU ---------------------------------------------------------------------------------------------------------- -->
<!-- ------------------------------------------------------------------------------------------------------------------------ -->
<?= $this->section('userGuru'); ?>
<section class="section">
    <div class="row mx-0 px-0">
        <div class="col px-0 mx-0">
            <h2 class="section-title mt-0 pt-2"><?= $judul ?></h2>
            <p class="section-lead">Username, nama lengkap, dan detail guru</p>
        </div>
        <div class="col px-0 mx-0">
            <form action="<?= base_url() ?>/guru/userGuru/" method="POST">
                <div class="input-group mb-3 pt-1">
                    <input name="keyword_guru" class="form-control" type="Search" placeholder="Cari..." aria-label="Search" data-width="250" style="border-radius: 20px 0 0 20px;">
                ...
                <div class="row mx-1">
                    <div class="col mt-2">
                        ...
                        <table class="table table-responsive table-striped table-bordered table-md">
                            <thead>
                                <tr>
                                    <th class="col-1 text-center text-primary">No</th>
                                    <th class="col-2 text-primary">Username</th>
                                    <th class="col-4 text-primary">Nama Lengkap</th>
                                    <th class="col-3 text-primary">Jabatan</th>
                                    <th class="col-2 align-middle text-primary">No Telp</th>
                                    <th class="col align-middle text-primary aksi">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                foreach ($dataTeachers as $dataTeacher) : ?>
                                    <tr>
                                        ...
                                        <td class="align-middle"><?= @ucwords($dataTeacher->username) ?></td>
                                        <td class="align-middle"><?= @ucwords($dataTeacher->fullname . ", " . $dataTeacher->title) ?></td>
                                        <td class="align-middle">
                                            <?php if (empty($dataTeacher->jabatan)) : ?><span class="text-danger">&mdash;</span>
                                            <?php else : echo @ucwords($dataTeacher->jabatan);
                                            endif; ?>
                                        </td>
                                        <td class="align-middle"><?= ($dataTeacher->nomorHp) ?></td>
                                        <td class="align-middle">
                                            <!-- Lihat -->
                                            <form action="<?= base_url() ?>/guru/detail/<?= ($dataTeacher->username) ?>" method="POST" class="d-inline">
                                                <?= csrf_field() ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form action="<?= base_url() ?>/guru/delete/<?= (@$dataTeacher->username) ?>" method="POST" class="d-inline">
                    <?= csrf_field() ?>
