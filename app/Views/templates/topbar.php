...
<nav class="navbar navbar-expand-lg main-navbar">
    ...
    <ul class="navbar-nav navbar-right">
        <?php if (@session()->logged_in) : ?>
            ...
            <!-- ICON USER POJOK KANAN ATAS -->
            <li class="dropdown"><a href="#" data-toggle="dropdown" class="nav-link dropdown-toggle nav-link-lg nav-link-user">
                    <img alt="image" src="<?= base_url(); ?>/templates/assets/img/avatar/avatar-1.png" class="rounded-circle mr-1">
                    <div class="d-sm-none d-lg-inline-block"><?php if (session()->levelId == 3) : echo @ucwords(session()->firstname['0']);
                                                                else : echo @ucwords(session()->data->username);
                                                                endif; ?></div>
                </a>
                <div class="dropdown-menu dropdown-menu-right">
                    <div class="dropdown-title">
                        <?php if (session()->levelId == 3) : echo @ucwords(session()->firstname['0']) . " " . @ucwords(session()->firstname['1']);
                        else : echo @ucwords(session()->data->username);
                        endif; ?>
                    </div>
                    <a href="<?= base_url();
                                if (session()->levelId == 1) : ?>/admin/profile/<?= $id = session()->id ?>
                                <?php elseif (session()->levelId == 2) : ?>/guru/profile/<?= $id = session()->id ?>
                                    <?php else : ?>/siswa/profile/<?= $id = session()->id ?>
                                        <?php endif; ?>" class="dropdown-item has-icon">
                        <i class="far fa-user"></i> Profile
                    </a>
                    <a href="<?= base_url(); ?>/siswa/detail" class="dropdown-item has-icon">
        ...
        <?php else : ?>
            ...
            <a href="<?= base_url('login'); ?>" class="nav-link nav-link-user">
                <div class="d-sm-none d-lg-inline-block mt-1">Login</div>
            </a>
